...
bool gx_gui_open_display(gx_hotboxUI *ui) {
	return true; // STUB
}

void gx_gui_create_window_and_surface(gx_hotboxUI *ui) {
	// prepare window class
	static TCHAR szClassName[] = TEXT("gx_hotboxUIClass");
	WNDCLASS wndclass = {0};
...
void gx_gui_register_controller_message(gx_hotboxUI *ui) {
	// dummy, not required on MSWin
}

void gx_gui_destroy_main_window(gx_hotboxUI *ui) {
	DestroyWindow(ui->win);
	// safe to use: doesnt unregister if there are still windows of this class
	UnregisterClass(TEXT("gx_hotboxUIClass"), NULL);
}

void gx_gui_resize_surface(gx_hotboxUI *ui) {
	RECT rect;
...
void gx_gui_send_controller_event(gx_hotboxUI *ui, int controller) {
...
void event_handler(gx_hotboxUI *ui) {
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	gx_hotboxUI *ui = (gx_hotboxUI *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
