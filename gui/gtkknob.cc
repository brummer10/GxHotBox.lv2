...
#define GTK_HOTBOX_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_HOTBOX_KNOB, GtkHotBoxKnobPrivate))

G_DEFINE_TYPE(GtkHotBoxKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkHotBoxKnobPrivate GtkHotBoxKnobPrivate;

struct _GtkHotBoxKnobPrivate
{
    int knob_x;
    int knob_y;
    int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
    /** check resize **/
    int grow;
    GtkHotBoxKnobPrivate *priv = GTK_HOTBOX_KNOB_GET_PRIVATE(widget);
    ...
    if (priv->model != 3) {
        ...
        GtkHotBoxKnob *knob = GTK_HOTBOX_KNOB(widget);
...
{
    g_assert(GTK_IS_HOTBOX_KNOB(widget));
    GtkHotBoxKnob *knob = GTK_HOTBOX_KNOB(widget);
    GtkHotBoxKnobPrivate *priv = GTK_HOTBOX_KNOB_GET_PRIVATE(knob);
...
{
    GtkHotBoxKnob *knob = GTK_HOTBOX_KNOB(object);
...
/****************************************************************
 ** init the GtkHotBoxKnobClass
 */

static void gtk_knob_class_init (GtkHotBoxKnobClass *klass)
{
...
//--------- connect the events with funktions
    ...
    g_type_class_add_private(obj_class, sizeof (GtkHotBoxKnobPrivate));
}
...
static void gtk_knob_init (GtkHotBoxKnob *knob)
{
...
{
    GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_HOTBOX_KNOB, NULL ));
