...
static void ref_value_changed(GtkAdjustment *adj, gpointer* args) {
    ...
    gx_hotboxUI* ui = (gx_hotboxUI*)arg->ui;
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor*   descriptor,
    ...
    *widget = NULL;
    ...
    set_expose_func(GX_HOTBOX_PAINT_BOX(ui->pbox),"pedal_expose");
    ui->box = gtk_vbox_new(FALSE, 4);
    ui->hbox = gtk_hbox_new(TRUE, 4);
    ...
    gtk_box_pack_start(GTK_BOX(ui->box), ui->vbox, TRUE, TRUE, 60);
    ...
    set_expose_func(GX_HOTBOX_PAINT_BOX(ui->pf1box),"frame_expose");
    ...
    gtk_box_pack_start(GTK_BOX(ui->pf1box), ui->bp_box, TRUE, TRUE, 0);
    ...
    ui->adj[1] = gtk_adjustment_new( 0.25, 0.0, 1.0, 0.01, 0.01*10.0, 0);
    ui->knob[1] = gtk_knob_new_with_adjustment(GTK_ADJUSTMENT(ui->adj[1]));
    ui->label[1] = gtk_label_new("GAIN");
    ...
    gtk_box_pack_start(GTK_BOX(ui->pf1box), ui->vkbox[1], TRUE, TRUE, 0);
