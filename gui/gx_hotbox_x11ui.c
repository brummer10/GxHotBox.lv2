...
static void resize_event(gx_hotboxUI *ui);
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_hotboxUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const struct _LV2UI_Descriptor * descriptor,
	...
	gx_hotboxUI* ui = (gx_hotboxUI*)malloc(sizeof(gx_hotboxUI));
	...
	ui->controls[1] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 120, 60, 61, 61}, false,"GAIN", KNOB, GAIN};
	ui->controls[2] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 190, 60, 61, 61}, false,"BASS", KNOB, BASS};
	ui->controls[3] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 260, 60, 61, 61}, false,"TREBLE", KNOB, TREBLE};
	ui->controls[4] = (gx_controller) {{ 0.25, 0.25, 0.0, 1.0, 0.01},{ 330, 60, 61, 61}, false,"VOLUME", KNOB, VOLUME};
	ui->block_event = -1;
...
static void cleanup(LV2UI_Handle handle) {
	gx_hotboxUI* ui = (gx_hotboxUI*)handle;
	cairo_destroy(ui->cr);
...
static void knob_expose(gx_hotboxUI *ui,gx_controller* knob) {
	...
	if (knob->is_active == true) {
		cairo_set_source_rgba (ui->crf, 0.6, 0.6, 0.6,0.8);
	} else {
		cairo_set_source_rgba (ui->crf, 0.6, 0.6, 0.6,0.6);
	}
...
static void bypass_expose(gx_hotboxUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
	if(switch_->adj.value > 0.9)
		cairo_set_source_surface (ui->crf, ui->pswitch_on, 2.5, 2.5);
	else
		cairo_set_source_surface (ui->crf, ui->pswitch_off, 2.5, 2.5);
	cairo_paint (ui->crf);
...
static void draw_controller(gx_hotboxUI *ui, gx_controller* controller) {
...
static void _expose(gx_hotboxUI *ui) {
	static const char* plug_name = "GxHotBox" ;
	...
	cairo_scale (ui->cr, 1.9, 3.0);
	cairo_set_source_surface (ui->cr, ui->frame1, 10, 12);
	cairo_paint (ui->cr);
	cairo_scale (ui->cr, 1/1.9, 1/3.0);
...
static void controller_expose(gx_hotboxUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
static void send_controller_event(gx_hotboxUI *ui, int controller) {
...
static void check_value_changed(gx_hotboxUI *ui, int i, float* value) {
...
static void check_is_active(gx_hotboxUI *ui, int i, bool set) {
...
bool aligned(int x, int y, gx_controller *control, gx_hotboxUI *ui) {
...
static void scroll_event(gx_hotboxUI *ui, int direction) {
	float value;
	for (int i=0;i<CONTROLS;i++) {
...
static void set_key_value(gx_hotboxUI *ui, int set_value) {
	float value = 0.0;
	for (int i=0;i<CONTROLS;i++) {
...
static void key_event(gx_hotboxUI *ui, int direction) {
	float value;
	for (int i=0;i<CONTROLS;i++) {
		if (ui->controls[i].is_active) {
			value = min(ui->controls[i].adj.max_value,max(ui->controls[i].adj.min_value, 
...
static void set_previous_controller_active(gx_hotboxUI *ui) {
	for (int i=0;i<CONTROLS;i++) {
...
static void set_next_controller_active(gx_hotboxUI *ui) {
	for (int i=0;i<CONTROLS;i++) {
		if(ui->controls[i].is_active == true) {
			ui->controls[i].is_active = false;
			send_controller_event(ui, i);
...
void get_active_controller(gx_hotboxUI *ui, bool set) {
...
static void motion_event(gx_hotboxUI *ui, double start_value, XMotionEvent *mo) {
...
static void event_handler(gx_hotboxUI *ui) {
	XEvent xev;
